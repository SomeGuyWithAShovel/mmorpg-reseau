...
pub fn system_input_keyboard(
    keycodes_input: Res<ButtonInput<KeyCode>>,
    keys_input: Res<ButtonInput<Key>>, 
    mut msg_writer : MessageWriter<MessagePlayerAction>
)
{
    for action in &PlayerAction::ALL
    {
        if keycodes_input.pressed(action.get_key_code()) // TODO : the Action->KeyCode should probably be a resource
        {
...
pub fn messages_input_keyboard(
